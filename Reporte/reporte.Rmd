...
dado el criterio del WAIC, el mejor modelo es el \textit{Poisson} con liga exponencial. Es por esta razón que para los siguientes modelos se utilizará la verosimilitud \textit{Poisson} con liga \textit{exponencial}.
...
<!-- Se seleccionó el modelo 3 (Modelo de intercepto variante por estado y división y pendientes fijas) para modelar la tasa de asesinatos en EUA. En este modelo se agregan las 7 variables explicativas que muestran tener un impacto sobre la tasa de asesinatos. -->

<!-- En el primer esquema, se ubican las tasas base de asesinatos por cada $100,000$ habitantes $100,000e^{\beta_0^s}$ para cada uno de los 48 estados. Se observa que hay mucha homogeneidad de los interceptos en cuestiones de intervalos de confianza, lo cual llega a pensar que controlando por las covariables usadas, los estados no son tan distintos. Sin embargo, Existen estados comom Kansas, Missouri, Nevada y New México que están por encima del promedio de los parámetros  -->
...
tabla_modelo %>%
  ...
  mutate(
    Parámetros = 
      paste(
        "$\\beta_{0,\\text{",
        estados$Name[as.numeric(gsub(".*\\[(.*)\\]", "\\1", parametros))],
        "}}$",
        sep = ""
      )
  ) %>% 
